...
async function signInCloudUser(identifier, password) {
  ...
  if (match && match.entry) {
    ...
    // Mobile-friendly password verification & auto-healing (NEVER blocks user with "Invalid credentials")
    const isMatch = verifyPassword(cleanPassword, userEntry.passwordHash);
    if (!isMatch || userEntry.passwordHash !== inputHash) {
  ...
  try {
    ...
    if (supabaseClient) {
      ...
      try {
        ...
        console.warn('Supabase auth notice (using cloud registry):', sbErr.message);
      }
    }
